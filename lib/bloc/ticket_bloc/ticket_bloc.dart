...
class TicketBloc extends Bloc<TicketEvent, TicketState> {
  ...
  TicketBloc({required this.ticketRepository}) : super(TicketInitial()) {
    on<TicketInitialEvent>((event, emit) async {
      var wasSuccessfullTicket = await ticketRepository.prepareDio(event.envi);
      //var wasSuccessfullAn = await analyticsRepository.prepareDio(event.envi);
    ...
    on<TicketRebuildFinanceEvent>((event, emit) async {
      ...
      var result =
          await ticketRepository.rebuildFinance(event.dateStart, event.dateEnd);
      if (!result) {
    ...
    on<TicketSoldEventFinanceRebuildEvent>((event, emit) async {
      ...
      try {
        ...
        emit(TicketSoldEventFinanceRebuildFailedState(
